...
In this project, we evaluate compact instruction-tuned language models for local natural-language-to-SQL generation over embedded SQLite databases. We use the Spider 1.0 development set, direct SQLite execution, 4-bit NF4 quantization, lightweight SQL verification, and four schema-context/repair conditions.
...
We do not currently include a `requirements.txt` file in the uploaded artifact. The notebooks include the setup cells we used for the Colab experiments.

---

## Data setup for reproduction

We use the **Spider 1.0 development set** and the original Spider SQLite database files. Download these files separately from the official Spider release and place them locally in this layout:
...
The notebooks need access to the Spider development examples, Spider schema metadata, and the original SQLite database files. We should not redistribute the SQLite databases unless redistribution rights are explicitly confirmed.
...
This is the main benchmark notebook. It runs the end-to-end experimental pipeline:
...
This support notebook creates paper materials. It does **not** run full model inference. We use it to generate benchmark-design materials such as prompt listings, schema-context examples, prompt/context-length summaries, verification/repair diagrams, example tables, and paper-ready LaTeX snippets.
...
We do not redistribute model weights. The notebooks load the models from their original model repositories.
...
Condition D differs from Condition C only because it allows one verification-triggered repair attempt. Executable but semantically wrong SQL does not trigger repair.

---

## Released Dataset

We release the generated benchmark outputs on Kaggle:
...
Some files include Spider-derived fields such as natural-language questions, gold SQL, database identifiers, and schema-context information. Because of this, users should also cite the original Spider 1.0 paper when they use the dataset.
...
This pipeline runs from question and schema context to prompt construction, local small-language-model inference, candidate SQL extraction, lightweight verification, SQLite execution, and metric logging. We use the repair loop only in Condition D after verification failure.
...
This figure shows how schema-context choice changes prompt length. Conditions C and D are longer because we add types, keys, foreign-key information, and sample values.
...
This chart shows that one-step repair gives uneven gains. It helps most when the lightweight verifier can detect the errors.
...
7. Run the material-generation notebook if you need to regenerate paper figures or tables.
...
This benchmark uses the Spider 1.0 development set. Spider is a large-scale cross-domain semantic parsing and Text-to-SQL dataset with 10,181 questions, 5,693 unique SQL queries, and 200 databases across 138 domains. The official Spider page describes Spider as a complex and cross-domain benchmark for natural-language interfaces to relational databases.
